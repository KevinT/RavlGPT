#!/usr/bin/env python3
"""
Domain LLM Diagnostic Analyzer

CRITICAL: This analyzer focuses ONLY on PROBLEM SPACE issues:
- WHAT the loop learns about its domain
- Domain model quality and evolution
- Verification criteria effectiveness
- Domain pattern recognition
- Business logic learning

DO NOT use this for execution/solution space issues. Use execution_llm_analyzer.py instead.

Uses an LLM to intelligently analyze domain learning failures and generate context-aware
diagnostic recommendations based on verification results, domain models, learned patterns,
and domain metrics.
"""
...
class DomainLLMAnalyzer:
    """
    Analyzes domain learning issues using an LLM for intelligent diagnostics
    ...
    def __init__(self, api_key: Optional[str] = None, prompts_dir: Optional[Path] = None):
        """
        Initialize domain LLM diagnostic analyzer
        ...
        # Load unified domain health diagnostic prompt
        self.prompt_domain_diagnostic = self._load_prompt("domain_health_diagnosis.md")
    ...
    def analyze_domain_health(
        ...
        """
        # Format comprehensive domain information for LLM
        domain_info = self._format_domain_info(domain_context)
        ...
        try:
            diagnosis_text = self.llm.complete(
                prompt=prompt,
                max_tokens=get_max_tokens('health_check_domain_analysis', 4096)
            )
    ...
    def _format_domain_info(self, domain_context: Dict[str, Any]) -> str:
        """
        Format raw loop learning files for LLM context.
        ...
        # Basic context
        if "loop_name" in domain_context:
            parts.append(f"**Loop Name:** {domain_context['loop_name']}")

        if "loop_dir" in domain_context:
            parts.append(f"**Loop Directory:** {domain_context['loop_dir']}")

        # Raw file contents - let LLM analyze structure and content
        files = domain_context.get("files", [])
        if files:
            parts.append(f"\n**Loop Learning Files:** ({len(files)} files found)")
            parts.append("\nThe following files were found in loop_learning/. Analyze their contents to assess domain health:\n")
            ...
            parts.append("\n**No loop learning files found.** Loop may not have run yet.")

        return "\n".join(parts)

    def _format_learned_patterns(self, patterns: List[Dict[str, Any]]) -> str:
        """Format previous successful domain learning fixes as few-shot examples"""
        if not patterns:
            return "No previous domain patterns available"

        formatted = ["## Previously Successful Domain Learning Fixes:\n"]

        for pattern in patterns[:3]:  # Show top 3 examples
            formatted.append(f"**Pattern:** {pattern.get('id', 'unknown')}")
            formatted.append(f"- Domain Issue: {pattern.get('issue_summary', 'N/A')}")
    ...
    def _load_prompt(self, filename: str) -> str:
        ...
        if not prompt_file.exists():
            raise FileNotFoundError(f"Domain prompt file not found: {prompt_file}")
