#!/usr/bin/env python3
"""
RAVL-SYNC-CLAUDE - Sync RAVL commands to Claude Code

Creates and updates Claude Code slash commands for all RAVL CLI tools.

Usage:
    ravl-sync-claude [options]
...
Description:
    This command ensures .claude/commands/ contains up-to-date slash commands
...
Examples:
    ravl-sync-claude              # Interactive sync (prompts for overwrites)
    ravl-sync-claude --check      # Check status only
    ravl-sync-claude --force      # Force update all
"""
...
# __file__ is .ravl/ravl/bin/*.py, so parent.parent.parent gives .ravl/
...
class RAVLSyncClaudeCommand(RAVLCLIBase):
    """Sync RAVL commands to Claude Code"""
    ...
    def __init__(self):
        ...
        self.ravl_bin_dir = self.find_framework_root() / 'ravl' / 'bin'
    ...
    def run(self, args: argparse.Namespace):
        ...
        legacy_file = self.claude_commands_dir / 'list-ravls.md'
    ...
    def _discover_commands(self) -> Dict[str, Dict[str, str]]:
        ...
        for cmd_file in sorted(self.ravl_bin_dir.iterdir()):
            ...
            # Skip commands containing 'opencode'
            if 'opencode' in cmd_file.name.lower():
                continue
    ...
    def _sync_command(
        ...
        """
        cmd_file = self.claude_commands_dir / f'{cmd_name}.md'
...
"""
    ...
    def _handle_legacy_rename(self, legacy_file: Path, args: argparse.Namespace):
        ...
        new_file = self.claude_commands_dir / 'ravl-list.md'
    ...
    def _print_summary(self, results: List[Tuple[str, str]], check_only: bool):
        ...
        """
        print("", file=sys.stderr)
        self.print_header("Claude Code Command Sync", "🔄")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Sync RAVL commands to Claude Code',
