...
class LoopContextBuilder:
    ...
    def build_context_summary(self, reflection: Dict[str, Any]) -> str:
        ...
        if child_metadata:
            ...
            context_parts.append("# - RAVL_COMMAND: Python interpreter (ensures same venv as parent)")
            context_parts.append("# - RAVL_SCRIPT: Path to ravl.py script")
            context_parts.append("# - RAVL_CWD: The working directory")
            context_parts.append("#")
            context_parts.append("# Execute child loops with: [RAVL_COMMAND, RAVL_SCRIPT, qualified_name]")
            context_parts.append("")
